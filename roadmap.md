...
프롬프트에서 도구 목록이 빠졌다. **다만 이것을 "절감"이라고 적었던 것은 틀렸다** —
아래 §1-1a에서 실측으로 바로잡는다.
...
gemma3:4b는 tools가 없어 텍스트 경로로 간다.
...
### ✅ 1-1a. 앞의 두 항목이 적어 둔 숫자를 실측으로 바로잡음
이 문서가 §0에서 *"의존성 핀은 추측하면 안 된다, 돌아가는 환경에서 읽어야 한다"* 고
경고해 놓고, 정작 자기 성과를 그렇게 적었다. 세 가지가 틀렸다.

| 적혀 있던 것 | 실측 | 왜 틀렸나 |
|:---|:---|:---|
| `17.8KB → 5.8KB` / `17.8KB → 5.1KB` | — | **같은 변경에 숫자가 두 개다** |
| "프롬프트가 줄었다(매 턴)" | 네이티브가 **더 비싸다** | 스키마는 사라진 게 아니라 `tools` 필드로 옮겨갔고, 그것도 매 턴 보내고 매 턴 토큰화된다 |
| 기준값 `17.8KB` | 지금 **23.7KB** | deepthink·channel·vm·verify가 그 뒤에 들어왔다. 33% 자랐다 |

지금 이 저장소에서 잰 값:

```
텍스트 모드 :                 5,928 토큰
네이티브    : 2,040 + 4,332 = 6,372 토큰   ← 더 비싸다
```

`config.py`는 처음부터 정직했다 — *"Choose this for accuracy, not to save
context."* **config.py가 맞고 이 문서가 틀렸다.** 네이티브를 택하는 이유는
정확성이지 컨텍스트가 아니다.

### ✅ 1-1b. MCP 서버 지연 로딩 — 진짜 부풀어오르던 곳
위를 재다가 나온 것. 컨텍스트를 먹는 건 내장 도구가 아니라 **MCP**였다.
`@playwright/mcp` 서버 하나(도구 24개)가 매 요청에 더하는 비용:
...
브라우저와 아무 상관 없는 대화에서도 매 턴 냈다. 서버 셋을 붙이면 65,536 컨텍스트의
절반이 아무도 안 쓴 도구 설명이다.

그래서 도구가 `MCP_LAZY_MIN_TOOLS`(6) 이상인 서버는 **설명하지 않고 이름만 알린다.**
모델이 필요하다고 판단하면 `use_mcp_server`가 파라미터를 넘긴다 — 스킬이 이미 하고 있던
모양 그대로다. 실측으로 playwright 하나에서 프롬프트 3,549 → **186**, 네이티브 스키마
4,637 → **0**이 됐다.

**이건 권한이 아니다.** 안 불러온 서버의 도구를 호출해도 `dispatch_tool`은 그대로
실행하고, 그 호출이 그 서버를 불러온다. 호출을 거절할 수 있는 절감은 절감이 아니다.

### ✅ 2-1. 자동 테스트-디버그 루프 — `verify.py`
파일을 고친 모델은 **"다 됐습니다"라고 말한다. 아무것도 돌려보지 않고서.**
게으른 게 아니라 로컬 4B 모델에게는 애초에 그 생각이 안 떠오른다 —
시스템 프롬프트에 "네 작업을 확인하라"고 써 놔도 세 번째 도구 호출에서 잊는다.
그래서 사람이 직접 돌려보고, 트레이스백을 복사해서, 다시 붙여넣는다.
**이 하네스로 사람이 가장 많이 반복하는 일이 그 왕복이다.** 이제 하네스가 한다.

한 턴이 파일을 바꾸면, 그 턴의 도구 호출이 **전부 끝난 뒤에** 프로젝트가
스스로 선언해 둔 검사를 돌리고, 실패하면 모델 앞에 갖다 놓는다.
구체적인 트레이스백에 반응하는 건 작은 모델이 잘하는 종류의 일이다 —
시키지도 않았는데 뭔가 잘못됐을지 모른다고 알아채는 것보다 훨씬 쉽다.

**검사를 지어내지 않는다.** 마커 파일이 있어야 하고(`pyproject.toml`,
`package.json`, `Cargo.toml`, `go.mod`), 러너가 설치돼 있어야 하고,
npm은 `test` 스크립트가 `npm init`이 써 놓은 자리표시자가 아니어야 한다.
**무엇을 돌릴지는 바뀐 파일의 확장자가 정한다** — 먼저 찾은 마커가 아니라.
한 저장소에 `pyproject.toml`과 `package.json`이 둘 다 있어도 `.py`는 pytest,
`.ts`는 npm으로 간다. 우선순위 규칙을 틀릴 여지가 없다.

**한 턴에 한 번이다.** 한 응답에서 파일 네 개를 고쳤으면 스위트는 한 번 돈다 —
중간 상태 셋이 아니라 모델이 남기려던 상태에 대고.

**경계가 있고, 포기한다.** `VERIFY_TIMEOUT`초, stdin 없음, 출력은 실패가
적혀 있는 꼬리만. 타임아웃을 넘긴 스위트는 매 편집마다 그 값을 물리느니
**스스로 꺼진다**(`/autoverify on`으로 다시 시도). 그리고 3연속 실패하면
되먹이기를 멈추고 "무엇이 깨졌는지 설명하라"고 시킨다 —
네 번째 추측은 사용자에게 정직한 설명보다 값어치가 없다.

**2-1이 1-3 뒤에 온 이유가 여기서 갚아진다.** 재시도마다 자기 커밋이 남으므로
루프가 엉뚱하게 갔어도 `/undo`로 한 걸음씩 되돌린다.

실패는 tool result가 아니라 **메시지**로 들어간다. `[System]`으로 시작하는
tool result는 `chat_turn`이 "닫힌 문을 두드리는 모델"로 세어 3회에 턴을 끊는데,
이건 정반대 — 들어가라고 열어 준 문이다.
...
gantt
    ...
    section 기준선
    테스트-디버그 루프       :done, b3, 2026-09-05, 1d
